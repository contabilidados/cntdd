...
#'
#' @description
#' Essa função calcula o Prazo Médio de Pagamento de Compras da empresa
#'
#' @details
#' Representa a quantidade média de dias que a empresa recebe de prazo para
...
#'
#' 3.  **Índice** o PMPC dos períodos informados;
#'
...
#' Ao acessar, fazer busca pelo nome da função `ind_pmpc`
#'
...
#' @param fornecedoresInicial Vetor com os valores de fornecedores do período anterior ao da análise
#' @param fornecedoresFinal Vetor com os valores de fornecedores do período da análise
...
#'  indicador = "PMPC",
#'  periodo = 2021:2022,
#'  fornecedoresInicial = c(2000, 1974),
#'  fornecedoresFinal = c(1930, 2400),
...
ind_pmpc <-
  function(
    ...
    if(giro){
      ratio <-
        round((cmv + estoqueFinal - estoqueInicial) /((fornecedoresInicial + fornecedoresFinal)/2), 4)
    } else {
      ratio <-
        round(((fornecedoresInicial + fornecedoresFinal)/2) / (cmv + estoqueFinal - estoqueInicial) * qdeDiasAno, 4)
    }
  ...
  dtGeral <-
    data.frame(
      periodo         = periodo,
      fornecedoresInicial = fornecedoresInicial,
      fornecedoresFinal   = fornecedoresFinal,
    ...
    arrange(periodo) %>%
    mutate(
      AH.fornecedoresInicial = round(fornecedoresInicial / lag(fornecedoresInicial) - 1, 4),
      AH.fornecedoresFinal   = round(fornecedoresFinal / lag(fornecedoresFinal) - 1, 4),
      ...
      AH.receitaLquida    = round(receitaLiquida / lag(receitaLiquida) - 1, 4),
      AH.atvTotal        = round(atvTotal / lag(atvTotal) - 1, 4),
      AV.fornecedoresInicial = fornecedoresInicial / atvTotal,
      AV.fornecedoresFinal   = fornecedoresFinal / atvTotal,
