...
#'
#' @description
#' Essa função calcula o Índice de Liquidez Geral baseado em vetores relativos às
#' contas de ativo circulante / não circulante  e passivo circulante / não circulante.
#'
#' @details
...
#'
#' 3.  **Índice** o índice de Liquidez Geral dos períodos informados;
#'
...
#' Ao acessar, fazer busca pelo nome da função `ind_liqGeral`
#'
...
#' @param atvRealizLP Vetor com os valores do ativo realizável a longo prazo (Ativo Não Circulante)
...
#' @param pasNaoCirc Vetor com os valores do passivo não circulante
...
#'  atvTotal = c(900,800),
#'  relatorio = TRUE,
#'  titulo = "Evolucao da Liquidez Geral",
#'  subtitulo = "",
#'  rodape = "",
#'  corFundo = "orange",
...
ind_liqGeral <-
  ...
  ratio <-
    round((cxEquiv + estoque + ctaRecCP + outAtvCirc + atvRealizLP)  /
    (fornec + dividasCP + outPasCirc + pasNaoCirc), 4)
  ...
  dtGeral <-
    data.frame(
      ...
      atvRealizLP = atvRealizLP,
      fornec      = fornec,
      dividasCP   = dividasCP,
      outPasCirc  = outPasCirc,
      pasNaoCirc  = pasNaoCirc,
      atvTotal    = atvTotal
    ) %>%
    mutate(
      ratio = ratio
    ...
    mutate(
      ...
      AH.Estoque    = round(estoque / lag(estoque) - 1, 4),
      AH.ctaRecCP   = round(ctaRecCP / lag(ctaRecCP) - 1, 4),
      AH.outAtvCirc = round(outAtvCirc / lag(outAtvCirc) - 1, 4),
      AH.atvRealizLP = round(atvRealizLP / lag(atvRealizLP) - 1, 4),
      AH.fornec     = round(fornec / lag(fornec) - 1, 4),
      AH.dividasCP  = round(dividasCP / lag(dividasCP) - 1, 4),
      AH.outPasCirc = round(outPasCirc / lag(outPasCirc) - 1, 4),
      AH.pasNaoCirc = round(pasNaoCirc / lag(pasNaoCirc) - 1, 4),
