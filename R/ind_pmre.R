...
#'
#' 3.  **Índice** o PMRE dos períodos informados;
#'
...
#' Ao acessar, fazer busca pelo nome da função `ind_pmre`
#'
...
ind_pmre <-
  function(
    ...
    if(giro){
      ratio <-
        round(cmv /((estoqueInicial + estoqueFinal)/2), 4)
    } else {
      ratio <-
        round(((estoqueInicial + estoqueFinal)/2) / cmv * qdeDiasAno, 4)
    }
