...
#'
#' @description
#' Essa função calcula a imobilização do patrimônio líquido baseado em vetores
#' relativos as contas de investimentos, imobilizado, intangíveis e patrimônio
#' líquido.
#'
#' @details
...
#'
#' 3.  **Índice** a imobilização do patrimônio líquido dos períodos informados;
#'
...
#' Ao acessar, fazer busca pelo nome da função `ind_imobilizacaoPL`
#'
...
#' @param imobilizado Vetor com os valores do imobilizado da empresa
#' @param intangiveis Vetor com os valores dos intangíveis da empresa
#' @param patLiq Vetor com os valores de patrimônio líquido da empresa
...
#'
#' ind_imobilizacaoPL(
#'  indicador = "Imobilizacao do PL",
#'  periodo = 2021:2022,
#'  investimentos = c(8, 10),
#'  imobilizado = c(150, 200),
#'  intangiveis = c(230, 412),
#'  patLiq = c(300, 450),
#'  atvTotal = c(600, 700),
#'  relatorio = TRUE,
#'  titulo = "Evolucao da Imobilizacao do PL",
#'  subtitulo = "",
#'  rodape = "",
#'  corFundo = "orange",
...
ind_imobilizacaoPL <-
  ...
  ratio <-
    round((investimentos + imobilizado + intangiveis) / patLiq, 4)
  ...
  dtGeral <-
    ...
    mutate(
      ...
      AH.imobilizado    = round(imobilizado / lag(imobilizado) - 1, 4),
      AH.intangiveis    = round(intangiveis / lag(intangiveis) - 1, 4),
      AH.patLiq    = round(patLiq / lag(patLiq) - 1, 4),
      AH.atvTotal     = round(atvTotal / lag(atvTotal) - 1, 4),
      AV.investimentos    = investimentos / atvTotal,
      AV.imobilizado    = imobilizado / atvTotal,
      AV.intangiveis = intangiveis / atvTotal,
